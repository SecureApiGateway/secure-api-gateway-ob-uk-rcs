...
public class AccountConsentService implements AccountConsentServiceInterface {
    ...
    @Override
    public AccountConsentDetails getConsent(AccountConsentRequest accountConsentRequest) throws ExceptionClient {
        log.debug("Received an account access consent request with JWT: '{}'", accountConsentRequest.getConsentRequestJwtString());
        String accountIntendId = accountConsentRequest.getIntentId();
        log.debug("=> The account consent detailsRequest id: '{}'", accountIntendId);
        String clientId = accountConsentRequest.getClientId();
        log.debug("=> The client id: '{}'", clientId);

        AccountConsentDetails accountConsentDetails = request(accountConsentRequest.getIntentId(), GET, null);
        String errorMessage;
        if (accountConsentDetails == null) {
            ...
            throw new ExceptionClient(accountConsentRequest, ErrorType.NOT_FOUND, errorMessage);
        }
        ...
        if (!clientId.equals(accountConsentDetails.getOauth2ClientId())) {
            ...
            log.error(errorMessage);
            throw new ExceptionClient(accountConsentRequest, ErrorType.INVALID_REQUEST, errorMessage);
        }
    ...
    @Override
    public Consent updateConsent(AccountConsentDecision consentDecision) throws ExceptionClient {
